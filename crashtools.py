...
class DataPrintoutContext(Util):
	...
	def detectTypeGetName(self, a0, sbt):
		vtb0 = self.check(sbt.process.ReadPointerFromMemory(a0, self.error))
		...
		print("vtb0=%x sc='%s' cu='%s' %s" % (
			vtb0, sc, sc.GetCompileUnit(), sc.symbol))
		pp = sc.symbol.name.split(" for ")
		if (len(pp) > 1):
			return pp[1]
...
def printValue(val, trail, filter):
	indent=None
	if len(trail) > 40:
	...
	suppress = filter and not re.search(filter, prefix) and\
		not re.search(filter, type.name)
...
def printType(debugger, command, result, internal_dict):
	...
	if not (len(commandArguments) in [1,2,3]):
		print("usage: dt ADDR [TYPE] [REGEXP]")
		return
	...
	data = val.GetData()
	print("data=%s" % (data))
	filter = (len(commandArguments) >= 3) and re.compile(commandArguments[2])
	printValue(val, [], filter)
...
def summaryIcu(val, internal_dict):
	return icuOrError(val.GetLoadAddress(), val.GetProcess())
